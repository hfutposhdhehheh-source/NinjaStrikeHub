		...
		if plr ~= player then
			...
			if char then
				...
				if roleESP.murder and hasKnife then
					createESP(plr,Color3.fromRGB(255,0,0),"ไอ้ชั่ว")
				end

				if roleESP.sheriff and hasGun then
					createESP(plr,Color3.fromRGB(0,120,255),"นักปราบชั่ว")
				end

				if roleESP.innocent and not hasKnife and not hasGun then
					createESP(plr,Color3.fromRGB(0,255,120),"คนดี")
				end
...
-- =========================
-- AUTO SHOOT MURDERER THROUGH WALL
-- =========================

local autoShootWall = false
...
RunService.RenderStepped:Connect(function()
    ...
    if not autoShootWall then return end
